...
def get_keywords():
	text = request.args.get_json()['text']
	num_phrases = int(request.args.get_json()['num_phrases'])
	print(text)
	print(num_phrases)
	# load a spaCy model, depending on language, scale, etc.
	nlp = spacy.load("en_core_web_sm")
...
def get_wiki_keywords():
	...
	return jsonify({"final_phrases": final_phrases, "scraped_data": scraped_data, "recommended_phrases": rec_phrases})
